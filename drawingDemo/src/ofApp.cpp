...
void ofApp::draw() {
    ...
    if(mode == 2) rpiLogo.getTextureReference().bind();
    ...
    if(mode == 2) rpiLogo.getTextureReference().unbind();
    ...
    if(bInfoText) {
        stringstream ss;
        ss << "Framerate: " << ofToString(ofGetFrameRate())		<< "\n";
		ss << "KEYS "											<< "\n";
        ss <<"s: Draw Solid Shapes"							<< "\n";
		ss <<"w: Draw Wireframes"								<< "\n";
        ss <<"1-4: Set Resolutions"								<< "\n";
		ss <<"n: Draw Normals"									<< "\n";
		ss <<"e/r: Set Mode " << ofToString(mode, 0)			<< "\n";
        ss <<"z: Split Faces " << bSplitFaces					<< "\n";
        ss <<"a: Draw Axes"										<< "\n";
		ss <<"l: Render lights"									<< "\n";
		ss <<"t: Info Text"										<< "\n";
        
        ofDrawBitmapStringHighlight(ss.str(), 100, 100, ofColor::yellow, ofColor::black);
    }
	ofPopStyle();
...
void ofApp::keyPressed(int key) {
    ...
    if(mode == 2) {
        plane.resizeToTexture( rpiLogo.getTextureReference(), .5 );
        box.mapTexCoordsFromTexture( rpiLogo.getTextureReference() );
        sphere.mapTexCoordsFromTexture( rpiLogo.getTextureReference() );
        icoSphere.mapTexCoordsFromTexture( rpiLogo.getTextureReference() );
        cylinder.mapTexCoordsFromTexture( rpiLogo.getTextureReference() );
        cone.mapTexCoordsFromTexture( rpiLogo.getTextureReference() );
    }
